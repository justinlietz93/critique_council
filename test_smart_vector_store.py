#!/usr/bin/env python
"""
Test script for the smart vector store implementation.

This script demonstrates:
1. Setting up the smart vector store with automatic backend selection
...
logger = logging.getLogger("arxiv_smart_vector_demo")

# Ensure proper import paths
sys.path.insert(0, os.path.abspath('.'))
from src.arxiv.smart_vector_store import ArxivSmartStore

# Sample paper data for testing (same as in other tests)
SAMPLE_PAPERS = [
    {
        "id": "2109.00558v1",
...
def demo_smart_store(openai_api_key: str = None, force_fallback: bool = False):
    """
    Demonstrate the ArXiv smart vector store.
    
    Args:
        openai_api_key: OpenAI API key for embeddings
        force_fallback: Force use of the fallback implementation
    """
    logger.info("Starting ArXiv smart vector store demo")
    
    # Initialize the store with a test cache directory
    cache_dir = "storage/smart_demo"
    store = ArxivSmartStore(
        cache_dir=cache_dir,
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='ArXiv Smart Vector Store Demo')
    ...
    # Run the demo
    demo_smart_store(openai_api_key, args.force_fallback)
